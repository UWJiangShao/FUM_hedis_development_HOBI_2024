...
proc freq data= HEDIS_VALUE_SET; tables value_set_name_new; run;
...
/*proc sql;*/
/*    select count(*) as ed_after_filter_2022*/
/*    from (select distinct membno, dfrdos from star_enc_all_flagged_denom);*/
/*    title2;*/
/*quit;*/
...
/* check denominator: in 2022, the denominator should be 7413 for all age, this program gets 7885 */ 

/* numerator */
/* copy the membno, admin date and discharge date, mapped from denominator */ 
proc sql;
    create table star_ed_followup_22 as
    select a.*, 
	b.dfrdos_new, b.dtodos_new
    from star_enc_all_flagged as a
        join denom_noinp_no31_nohos as b
        on a.membno = b.membno
        and b.dfrdos_new <= a.dfrdos <= intnx('day', b.dtodos_new, 30)
    order by a.membno, a.dfrdos;
    ;
quit;

proc freq data=star_ed_followup_22; 
tables plancod; 
run;

%put &mental_diagn_icd10cm; 

proc freq data=star_ed_followup_22; 
tables diagn1 svccod poscod; 
run;

data flag_num_event_22;
	set star_ed_followup_22; 
		...
		* An intensive outpatient encounter or partial hospitalization (Partial Hospitalization or Intensive Outpatient Value Set) with 
        ...
        if (flag_outpatient = 1)
			and svccod in:(&part_hosp_inten_outp_hcpcs) 
		...
		* A community mental health center visit (Visit Setting Unspecified Value Set with 
		...
		and poscod in:(&comm_mental_health_pos) 
		and (is_MH = 1 
